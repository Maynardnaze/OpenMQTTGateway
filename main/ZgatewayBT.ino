...
double value_from_service_data(const char* service_data, int offset, int data_length, bool reverse) {
...
JsonObject& process_sensors(int offset, JsonObject& BLEdata) {
  ...
  double value = 9999;
  value = value_from_service_data(servicedata, 28 + offset, data_length, true);
  ...
  switch (servicedata[23 + offset]) {
    ...
    case 'd':
      // temperature
      value = value_from_service_data(servicedata, 28 + offset, 4, true);
      BLEdata.set("tem", (double)value / 10); // remove for 0.9.6 release
      BLEdata.set("tempc", (double)value / 10);
      BLEdata.set("tempf", (double)convertTemp_CtoF(value / 10));
      // humidity
      value = value_from_service_data(servicedata, 32 + offset, 4, true);
      BLEdata.set("hum", (double)value / 10);
      break;
    default:
...
JsonObject& process_scale_v1(JsonObject& BLEdata) {
  ...
  double weight = value_from_service_data(servicedata, 2, 4, true) / 200;
...
JsonObject& process_scale_v2(JsonObject& BLEdata) {
  ...
  double weight = value_from_service_data(servicedata, 22, 4, true) / 200;
  double impedance = value_from_service_data(servicedata, 18, 4, true);
...
JsonObject& process_inkbird(JsonObject& BLEdata) {
  ...
  double temperature = value_from_service_data(manufacturerdata, 0, 4, true) / 100;
  double humidity = value_from_service_data(manufacturerdata, 4, 4, true) / 100;
  double battery = value_from_service_data(manufacturerdata, 14, 2, true);
...
JsonObject& process_miband(JsonObject& BLEdata) {
  ...
  double steps = value_from_service_data(servicedata, 0, 4, true);
...
JsonObject& process_milamp(JsonObject& BLEdata) {
  ...
  long darkness = value_from_service_data(servicedata, 8, 2, true);
...
JsonObject& process_cleargrass(JsonObject& BLEdata, boolean air) {
  ...
  double value = 9999;
  // temperature
  value = value_from_service_data(servicedata, 20, 4, true);
  BLEdata.set("tem", (double)value / 10); // remove for 0.9.6 release
  BLEdata.set("tempc", (double)value / 10);
  BLEdata.set("tempf", (double)convertTemp_CtoF(value / 10));
  // humidity
  value = value_from_service_data(servicedata, 24, 4, true);
  BLEdata.set("hum", (double)value / 10);
  if (air) {
    // air pressure
    value = value_from_service_data(servicedata, 32, 4, true);
